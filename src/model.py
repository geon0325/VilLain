...
class model(nn.Module):
    def __init__(self, V_idx, E_idx, V, E, num_subspace, dim_subspace, num_step, num_step_gen, tau, gb_hard):
    ...
    def forward(self):
        ...
        _X0 = F.gumbel_softmax(_X0_pre.view(-1, self.num_subspace, self.dim_subspace), tau=self.tau, dim=2, hard=self.gb_hard)
    ...
    def get_node_embeds(self, node=True):
        with torch.no_grad():
            ...
            if node:
                ...
                #_X = torch.zeros(X0.view(-1, self.num_subspace, self.dim_subspace).reshape(self.V, -1).shape).to(X0.device)
            else:
